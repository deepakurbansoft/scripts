...
if (mysql_affected_rows($con) == 1) {
    ...
    mysql_query($con, "DELETE FROM user_data WHERE wait='yes'");
...
if (isset($_POST['p_name']) && isset($_POST['mem_type']) && isset($_POST['total']) && isset($_POST['age']) && isset($_POST['paid'])) {
    ...
    if (mysql_affected_rows($con) == 1) {
        while ($row = mysql_fetch_array($result, MYSQLI_ASSOC)) {
            ...
            mysql_query($con, "UPDATE user_data SET name='$full_name', address='$address', contact='$contact', email='$email', height='$height', weight='$weight', joining='$date', age='$age', proof='$proof', other_proof='$other_proof', sex='$sex' WHERE wait='yes'");
            $bal = $total - $paid;
            
            mysql_query($con, "INSERT INTO subsciption (mem_id,name,sub_type,paid_date,total,paid,expiry,invoice,sub_type_name,bal,exp_time,renewal)
VALUES ('$p_id','$full_name','$mem_type','$date','$total','$paid','$expiry','$invoice','$name_type','$bal','$exp_time','yes')");
            ...
            mysql_query($con, "UPDATE user_data SET wait='no' WHERE wait='yes'");
        }
    }
