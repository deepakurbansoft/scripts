...
extract($_POST);
	...
	if(_GET('paypal')=='checkout'){
		...
		*/
		...
		$token_id = substr($_SESSION['token_id'],3);
		...
		if(mysql_num_rows($user_query)>0) {
			...
			$insert_email = mysql_query("INSERT INTO `users` (`email`) VALUES ('$email')");
	...
	$_SESSION['cbpc_user'] = $user_id;
		...
		foreach($product_id as $product){
			$orders_query = mysql_query("INSERT INTO `orders` (`user_id`,`token`,`product_id`) VALUES ('$user_id','$token_id','$product')");
